/********************************** (C) COPYRIGHT *******************************
 * File Name          : gamepad_mapper.h
 * Author             : Antigravity
 * Version            : V1.0.0
 * Date               : 2026/04/24
 * Description        : Gamepad profile and mapping architecture.
 *******************************************************************************/
...
#define REGISTER_GAMEPAD_PROFILE(name, _vid, _pid, _process) \
    const GamepadProfile_t __profile_##name __attribute__((used, section("gamepad_profiles"))) = { \
        .vid = _vid, \
        .pid = _pid, \
        .process = _process \
    }
