/********************************** (C) COPYRIGHT *******************************
 * ファイル名          : usbc_source.c
 * 説明                : USB Type-C ソース (CC モニタリング + ロードスイッチ 制御)
 *
 * ハードウェア:
 ...
 *   シンクデバイス (5.1k Rd) が 接続 されると、一方の CC ピン の 電圧 が低下します。
 *   内部 コンパレータ (CC_CMP) を使用して、この スレッショルド の交差を 検出 します。
 *   ロードスイッチ は、デバウンス によって安定した アタッチメント が確認された後にのみ ON になります。
 *   デタッチメント 時には (短い デバウンス の後) 即座に OFF になります。
 *******************************************************************************/
 ...
 *   - オープン (デバイスなし): CC 電圧 ≈ VDD (≈ 3.3V) → すべての スレッショルド を上回る
...
/*
 * @fn      USBC_Source_Init
 * @brief   ソースモード 用に USBPD PHY を初期化し、ロードスイッチ GPIO を設定します。
 *
 * - CC1 (PC14) と CC2 (PC15) の両方で Rp (330uA) プルアップ を有効にします。
 * - ロードスイッチ GPIO (PA0) を 出力 として初期状態 OFF に設定します。
 * - PD 通信 (BMC) は有効にせず、CC レベル の 検出 のみを行います。
 */
void USBC_Source_Init(void)
{
    ...
    /* PD PHY を有効化 (アナログ セクション: プルアップ 電流源 & コンパレータ に必要) */
 ...
 * @brief   周期的な CC 検出。メインループ または タイマー から約 4ms ごとに呼び出します。
 *
 * ステートマシン:
 *   DISCONNECTED → CC1 または CC2 で Rd を 検出 → デバウンス → ATTACHED (ロードスイッチ ON)
 *   ATTACHED     → アクティブ な CC で オープン を 検出 → デバウンス → DISCONNECTED (ロードスイッチ OFF)
 */
void USBC_Source_Detect(void)
{
    ...
    if (s_state == USBC_SRC_DISCONNECTED)
    {
        ...
        uint8_t candidate = 0;
        if (cc1_state == 1)       candidate = 1;  /* CC1 で Rd */
        else if (cc2_state == 1)  candidate = 2;  /* CC2 で Rd */
